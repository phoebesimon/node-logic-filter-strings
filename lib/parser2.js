var Parsimmon = require('parsimmon');
var regex = Parsimmon.regex;
var string = Parsimmon.string;
var lazy = Parsimmon.lazy;
var seq = Parsimmon.seq;
var succeed = Parsimmon.succeed;
var alt = Parsimmon.alt;


function lexeme(parser) {
  return parser.skip(Parsimmon.optWhitespace);
}

function printAndReturn(msg, result) {
  return result;
}

// Parsimmon has this in git, but they haven't cut a release that includes it yet.
// https://github.com/jneen/parsimmon/blob/master/src/parsimmon.js#L106
function seqMap() {
  var args = [].slice.call(arguments);
  var mapper = args.pop();
  return seq.apply(null, args).map(function(results) {
    return mapper.apply(null, results);
  });
}


function commaSep(parser) {
  var commaParser = strings.comma.then(parser).many();

  return seqMap(parser, commaParser, function(first, rest) {
    return [first].concat(rest);
  }).or(succeed([]));
}


// Base strings
var strings = exports.strings = {
  lbrace: lexeme(string('{')),
  rbrace: lexeme(string('}')),
  lbrack: lexeme(string('[')),
  rbrack: lexeme(string(']')),
  lparen: lexeme(string('(')),
  rparen: lexeme(string(')')),
  colon: lexeme(string(':')),
  comma: lexeme(string(',')),
  and: lexeme(string('&&')),
  or: lexeme(string('||')),
  not: lexeme(string('!')),
  notEquals: lexeme(string('!==').or(string('!='))),
  equals: lexeme(string('===').or(string('=='))),
  number: lexeme(regex(/[0-9]+/).map(parseInt)),
  decimal: lexeme(regex(/[0-9]*\.?[0-9]+/).map(parseFloat))
};


// Tokens built out of base strings
var tokens = exports.tokens = {
  operator: strings.and.or(strings.or),

  quoted: lexeme(regex(/["']((?:\\.|.)*?)['"]/, 1).map(function(val) {
    return val.replace(/['"]+/g, '');
  }))
};

tokens.key = lexeme(tokens.quoted.or(regex(/[a-z_\.]+/i)).map(printAndReturn.bind(null, 'KEY')));

tokens.notKey = seqMap(strings.not, tokens.key, function(not, key) {
  return not + key;
});

tokens.pair = lazy('pair', function() {
  return seq(tokens.key.skip(strings.colon), tokens.value);
});

tokens.objectParse = lazy('object', function() {
  return seqMap(strings.lbrace, commaSep(tokens.pair), strings.rbrace, function(_, results, __) {
    var ret = {}, i;

    for (i = 0; i < results.length; i++) {
      ret[results[i][0]] = results[i][1];
    }
    return ret;
  });
});

tokens.array = lazy('array', function() {
 return seqMap(strings.lbrack, commaSep(tokens.value), strings.rbrack, function(_, results, __) {
   return results;
 });
});

tokens.value = tokens.objectParse.or(tokens.array).or(tokens.quoted).or(strings.decimal).or(strings.number);


//High Level
//i.e. 'foo === "bar"'
exports.condition = seq(
  tokens.key.map(printAndReturn.bind(null, 'KEY IN CONDITION')),
  strings.equals.or(strings.notEquals).map(printAndReturn.bind(null, 'EQUALS IN CONDITION')),
  tokens.value
).map(printAndReturn.bind(null, 'CONDITION'));

//i.e. '(foo === "bar")'
exports.parenCondition = lazy('parenCondition', function() {
  return strings.lparen.then(exports.condition.map(printAndReturn.bind(null, 'CONDITION INSIDE PAREN'))).skip(strings.rparen).map(printAndReturn.bind(null, 'PAREN CONDITION'));
});


//i.e. '!(foo === "bar")'
exports.notCondition = lazy(function() {
  return seqMap(
    strings.not,
    exports.parenCondition.or(exports.expression),
    function(not, condition) {
      if (condition.length === 1) {
        return [[not + condition[0]]];
      }
      return [not, condition];
    });
});

exports.anyCondition = alt(
  exports.condition.map(printAndReturn.bind(null, 'CONDITION IN ANY')),
  exports.parenCondition.map(printAndReturn.bind(null, 'PAREN IN ANY')),
  exports.notCondition.map(printAndReturn.bind(null, 'NOT IN ANY'))
).map(printAndReturn.bind(null, 'ANY CONDITION'));


exports.expression = lazy('an expression', function() {
  return alt(
    strings.lparen.then(exports.atom).skip(strings.rparen),
    exports.atom.map(printAndReturn.bind(null, 'ATOM IN EXPRESSION')),
    exports.form.map(printAndReturn.bind(null, 'FORM IN EXPRESSION'))
  ).map(function(result) {
    if (result instanceof Array) {
      return result.length === 1 ? result[0] : result;
    } else {
      return [result];
    }
  });
});


exports.atom = alt(
  exports.anyCondition.map(printAndReturn.bind(null, 'ATOM ANY CONDITION')),
  tokens.key.map(printAndReturn.bind(null, 'ATOM KEY')),
  tokens.notKey.map(printAndReturn.bind(null, 'ATOM NOT KEY'))
);


exports.form = lazy('format', function() {
  return strings.lparen.then(seq(exports.expression, tokens.operator, exports.expression)).skip(strings.rparen).map(printAndReturn.bind(null, 'FORM'));
});
//exports.form = strings.lparen.then(seq(exports.expression, tokens.operator, exports.expression).skip(strings.rparen));
/*alt(
  seq(strings.lparen, exports.expression, tokens.operator, exports.expression, strings.rparen),
  seq(exports.expression, tokens.operator, exports.expression)
);*/


exports.operand = seqMap(
  exports.expression,
  tokens.operator,
  exports.expression,
  function(expr1, op, expr2) {
    return [expr1, op, expr2];
  }
);


exports.filter = {
  parse: function(string) {
    string = '(' + string + ')';
    return exports.expression.parse(string);
  }
};
